...
def setup_model_peft(model_name: str, scratch_cache_dir: str, r: int):
...
folder_path = Path('datasets_csv/prompts_id_6')
...
for r in rs:
    model, tokenizer = setup_model_peft(model_name = MODEL_NAME,
                            scratch_cache_dir = scratch_cache_dir, r = r)
    
    tokenized_datasets = load_and_preprocess_data(str(filename_train_jsonl), str(filename_test_jsonl), tokenizer)

    training_args = TrainingArguments(output_dir=f'./checkpoints/lora_rank_{r}/',
