...
def evaluate_cross_graph(model: HookedTransformer, model2: HookedTransformer, graph: Graph, dataloader: DataLoader, 
                   metrics: Union[Callable[[Tensor],Tensor], List[Callable[[Tensor], Tensor]]], 
                   quiet=False, intervention: Literal['patching', 'zero', 'mean','mean-positional']='patching', 
                   intervention_dataloader: Optional[DataLoader]=None, skip_clean:bool=True) -> Union[torch.Tensor, List[torch.Tensor]]:
    """Evaluate a circuit (i.e. a graph where only some nodes are false, probably created by calling graph.apply_threshold). You probably want to prune 
    ...
    for clean, corrupted, label in dataloader:
        ...
        (fwd_hooks_corrupted_2, fwd_hooks_clean_2, _), activation_difference_2 = make_hooks_and_matrices(model2, graph, len(clean), n_pos, None)
        input_construction_hooks_2 = make_input_construction_hooks(activation_difference_2, in_graph_matrix, neuron_matrix)
        ...
        with torch.inference_mode():
            ...
            with model.hooks(fwd_hooks_clean + input_construction_hooks_2):
                logits = model(clean_tokens, attention_mask=attention_mask)
