...
def load_model(hf_model_name, translens_model_name, adapter_path = None, scratch_cache_dir = None):
...
for rank in ranks:
    for checkpoint in checkpoints:
        print(f'Rank:{rank} Checkpoint:{checkpoint}')
        prompt_id = f'lora_rank_{rank}'
        csv_id = f'prompts_id_6'
        results_folder = f'results_run_2_20-100/lora_rank_{rank}/checkpoint-{checkpoint}/'
        ...
        if checkpoint == 0:
            ...
            adapter_path = f"../../fine-tuning/lora_config/checkpoints/lora_rank_{rank}/checkpoint-{checkpoint}"
        model = load_model(
        ...
        ds = EAPDataset(f'../../fine-tuning/lora_config/datasets_csv/{csv_id}/test.csv')
        ...
        two_percent_edges = int(total_edges * 0.02)
        g.apply_topn(two_percent_edges , absolute=True, prune =True)
