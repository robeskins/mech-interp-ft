...
def get_circuit_logits(model: HookedTransformer, graph: Graph, data: tuple, prune:bool=True, quiet=False):
    ...
    # 为每个节点构建输入（非InputNodes），考虑attention heads的多输入
...
def evaluate_graph(model: HookedTransformer, graph: Graph, dataloader: DataLoader, metrics: Union[List[Callable[[Tensor], Tensor]],Callable[[Tensor], Tensor]], prune:bool=True, quiet=False):
    ...
    # def make_input_construction_hook(activation_differences, in_graph_vector, attn=False):
    #     def input_construction_hook(activations, hook):
    #         print(f"activation_differences shape: {activation_differences.shape}")
    #         print(f"in_graph_vector shape: {in_graph_vector.shape}")
            
    #         # 打印张量的数据类型
    #         print(f"activation_differences dtype: {activation_differences.dtype}")
    #         print(f"in_graph_vector dtype: {in_graph_vector.dtype}")
    #         in_graph_vector_expanded = in_graph_vector.squeeze(0)  # 去除多余的维度，变为 [16]

    #     # 扩展目标形状至 [64, 6, 409, 16]
    #         in_graph_vector_expanded = in_graph_vector_expanded.unsqueeze(0).unsqueeze(0).unsqueeze(0).expand(activation_differences.shape[0], 
    #                                                                                                            activation_differences.shape[1], 
    #                                                                                                            activation_differences.shape[2], 
    #                                                                                                            -1)  # 扩展至 [64, 6, 409, 16]
    #         # 计算 update
    #         if attn:
    #             update = torch.einsum(activation_differences[:, :, :len(in_graph_vector)], in_graph_vector, 
    #                                 'batch pos previous hidden, previous head -> batch pos head hidden')
    #         else:
    #             update = torch.einsum(activation_differences[:, :, :len(in_graph_vector)], in_graph_vector, 
    #                                 'batch pos previous hidden, previous -> batch pos hidden')

    #         # # 处理 activations 和 update 形状不匹配的情况
    #         # if activations.ndim == 4 and activations.shape[2] != update.shape[2]:
    #         #     # 特殊处理：当 s 的第二维是 8，scores 的第二维是 32
    #         #     # 填充 activations 的第二维到目标大小
    #         #     if activations.shape[2] == 8 and update.shape[2] == 32:
    #         #         # 将 activations 的第三维填充到 32
    #         #         padding = (0, 32 - activations.shape[2])  # 只在第三维填充
    #         #         activations = F.pad(activations, (0, 0, *padding))  # 填充第三维

    #         #     elif activations.shape[2] == 32 and update.shape[2] == 8:
    #         #         # 将 activations 的第三维填充到 8
    #         #         padding = (0, 8 - activations.shape[2])  # 只在第三维填充
    #         #         activations = F.pad(activations, (0, 0, *padding))  # 填充第三维
    #         #     else:
    #         #         raise ValueError(f"Cannot match dimensions: activations.shape={activations.shape}, update.shape={update.shape}")

    #         # 将 activations 和 update 相加
    #         activations += update
    #         return activations
