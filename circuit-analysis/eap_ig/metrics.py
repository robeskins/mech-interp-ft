...
def logit_diff(clean_logits: torch.Tensor, corrupted_logits: torch.Tensor, input_length: torch.Tensor, labels: torch.Tensor, mean=True, prob=False, loss=False):
    ...
    # 打印 cleans 和 labels 的维度
    print(f"cleans shape: {cleans.shape}")
...
def direct_logit(clean_logits, corrupted_logits, input_length, labels, mean=True, prob=False, loss=False):
    clean_logits = get_logit_positions(clean_logits, input_length)
    cleans = torch.softmax(clean_logits, dim=-1) if prob else clean_logits
    
    batch_size = cleans.size(0)
    good_bad = cleans[torch.arange(batch_size), labels.to(cleans.device)]
    ...
    # print(f"Cleans shape: {cleans.shape}")  # 应为 [batch_size, vocab_size]
    # print(f"Labels shape: {labels.shape}")  # 应为 [batch_size]
    # print(f"Good_bad shape: {good_bad.shape}")  # 应为 [batch_size]
    return results
