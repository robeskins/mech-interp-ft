...
def make_hooks_and_matrices(model: HookedTransformer, graph: Graph, batch_size:int , n_pos:int, scores: torch.Tensor, detach=True):
    ...
    def gradient_hook(prev_index: Union[slice, int], bwd_index: Union[slice, int], gradients:torch.Tensor, hook):
        ...
        try:
            ...
            # if s.ndim == 2 and s.shape[1] != scores[:prev_index, bwd_index].shape[1]:
            #     # 特殊处理：当 s 的第二维是 8，scores 的第二维是 32
            #     if s.shape[1] == 8 and scores[:prev_index, bwd_index].shape[1] == 32:
            #         # 这里不直接使用 expand，而是用 zero 填充的方式
            #         padding = (0, 32 - s.shape[1])  # 填充到 32
            #         s = F.pad(s, padding)  # 使用 PyTorch 的 pad 函数
            #     # 如果第二维是 32，scores 是 8，则扩展 s 的第二维
            #     elif s.shape[1] == 32 and scores[:prev_index, bwd_index].shape[1] == 8:
            #         padding = (0, 8 - s.shape[1])  # 填充到 8
            #         s = F.pad(s, padding)  # 使用 PyTorch 的 pad 函数
            #     else:
            #         raise ValueError(f"Cannot match dimensions: s.shape={s.shape}, scores.shape={scores[:prev_index, bwd_index].shape}")
