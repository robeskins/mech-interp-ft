...
def load_adapter_into_hooked_transformer(adapter_path, hf_model_name, translens_model_name, scratch_cache_dir = None):
...
def cross_task_faithfulness(models: list[HookedTransformer], graphs: list[Graph], dataloader: EAPDataset):
    
    accuracy_data = {}
    for model in models:
        for graph in graphs:
            accuracy_data['model'] 
            calculate_accuracy(model, graph, dataloader)

    for model in models:
        for graph in graphs:
            calculate_faithfulness(model, graph, dataloader)
