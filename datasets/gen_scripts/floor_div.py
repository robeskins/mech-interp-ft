...
def get_valid_combos(tokenizer):
    ...
    for a in range(2, 100):    
        for b in range(2, 100): 
            if a > b:
                result = a // b
                if is_single_token(str(result), tokenizer):  # exclude result == 1
                    div_combos.append((a, b))
    random.shuffle(div_combos)
    return div_combos
...
generate_csvs(prompts = prompts,
              split_percent = 0.9,
              results_dir = '../pythia-1.4B/sequential_dataset/FloorDiv')